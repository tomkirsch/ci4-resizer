...
class Resizer{
	...
	public function cleanDir(bool $forceCleanAll=FALSE){
		// interator
		...
		foreach ($iterator as $info) {
			...
			if($forceCleanAll || $expired){
				try{
					...
					log_message('error', 'Resizer Lib cannot unlink file '.$info->getRealPath());
				}
			}
		}
	}
	...
	public function cleanFile(string $imageFile){
		$dir = new \RecursiveDirectoryIterator($this->config->resizerCachePath);
		$filter = new \RecursiveCallbackFilterIterator($dir, function ($current, $key, $iterator) {
			// Skip hidden files and directories.
			if ($current->getFilename()[0] === '.') return FALSE;
	...
	protected function mimeFromExt(string $ext):?string{
		...
		switch(strtolower($ext)){
			case 'jpg':
			case 'jpeg':
				$mime = image_type_to_mime_type(IMAGETYPE_JPEG); break;
			case 'png':
				$mime = image_type_to_mime_type(IMAGETYPE_PNG); break;
			case 'webp':
				$mime = image_type_to_mime_type(IMAGETYPE_WEBP); break;
			case 'gif':
				$mime = image_type_to_mime_type(IMAGETYPE_GIF); break;
		}
		return $mime;
	}
